...
int runJogo(void)
{
    ...
    while (!WindowShouldClose())
    {
        ...
        if(recursos >= 2 && map[yPersonagem][xPersonagem] == '.' && IsKeyPressed(KEY_B)) {
            PlaySound(fxWav2);
            map[yPersonagem][xPersonagem] = 'W'; // Constrói uma barricada no local do personagem
        ...
        if (isPaused)
        {
            ...
            int spacing = 30; // Aumente o espaçamento entre os textos
            ...
            // Checa se o usuário pressiona N ou Q
            if (IsKeyPressed('N'))
            {
